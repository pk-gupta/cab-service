...
public class SubmitRequest implements ProcessRequest {

	@Override
	public ProcessNewRequestVO submit(InputDataMarker inputDataProvider, Object persistance) {
		ProcessNewRequestVO submitResult = null;
		SingleRequestVO singleRequestVO;
		EmployeeSingleRequestDAO employeeSingleRequestDAO;
		
		ScheduleRequestVO scheduleRequestVO;
		EmployeeScheduleRequestDAO employeeScheduleRequestDAO;
		
		if(inputDataProvider instanceof ScheduleRequestVO){
			scheduleRequestVO = (ScheduleRequestVO)inputDataProvider;
			...
			employeeScheduleRequestDAO = (EmployeeScheduleRequestDAO)persistance;
			performScheduleRequestValidation(scheduleRequestVO, employeeScheduleRequestDAO);
			submitResult = submitScheduleRequest(scheduleRequestVO, employeeScheduleRequestDAO);
		}else {
			singleRequestVO = (SingleRequestVO)inputDataProvider;
			employeeSingleRequestDAO = (EmployeeSingleRequestDAO)persistance;
			performSingleRequestValidation(singleRequestVO, employeeSingleRequestDAO);
			submitResult = submitSingleRequest(singleRequestVO, employeeSingleRequestDAO);
		}
	...
	private ProcessNewRequestVO submitSingleRequest(SingleRequestVO singleRequestVO, EmployeeSingleRequestDAO employeeSingleRequestDAO){
	...
	private ProcessNewRequestVO submitScheduleRequest(ScheduleRequestVO scheduleRequestVO, EmployeeScheduleRequestDAO employeeScheduleRequestDAO){
	...
	private void filterCreateUpdateRequestDetails(List<RequestDetailEntity> transactionReqDetailList,
			List<RequestDetailEntity> newRequestDetails, List<RequestDetailEntity> existingRequestDetails, RequestHeaderEntity inputRequestHeader) {
		
		RequestCRUDCriteria updateCriteriaList = new RequestCreateCriteria();
		List<RequestDetailEntity> updateRequestDetailsList = updateCriteriaList.meetCriteria(newRequestDetails, existingRequestDetails);
		if(!updateRequestDetailsList.isEmpty()){
			transactionReqDetailList.addAll(updateRequestDetailsList);
			newRequestDetails.removeAll(updateRequestDetailsList);
		}
